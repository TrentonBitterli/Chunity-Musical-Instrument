...
namespace UnityEngine.XR.Hands.Samples.VisualizerSample
{
    public class HandVisualizer : MonoBehaviour
    {
        ...
        public float xAxisMiddlePosition;
        public float xAxisRingPosition;
        public float xAxisLittlePosition;
        public float xAxisLPosition;
        public float xAxisLMiddlePosition;
        public float xAxisLRingPosition;
        public float xAxisLLittlePosition;
        ...
        void OnUpdatedHands(XRHandSubsystem subsystem, XRHandSubsystem.UpdateSuccessFlags updateSuccessFlags, XRHandSubsystem.UpdateType updateType)
        {
            ...
            if (updateType == XRHandSubsystem.UpdateType.Dynamic)
                ...
                // Debug.Log("Right Hand Pos: " + wristPose.position);
                // float xAxisPosition = wristPose.position.x;
                // float yAxisPosition = wristPose.position.y;
                // float zAxisPosition = wristPose.position.z;
                // Vector3 wristPosition = wristPose.position;
                // Debug.Log("X Axis: "+ xAxisPosition);
                // Debug.Log("Y Axis: "+ yAxisPosition);
                // Debug.Log("Z Axis: "+ zAxisPosition);
                // xAxisPosition = wristPosition.x;
                // Get the joint we are intereted in
                // var wrist = subsystem.rightHand.GetJoint(XRHandJointID.Wrist);
                // wrist.TryGetPose(out var wristPose);
                // //Make a vector3 of the wrist position.
                // Vector3 wristPosition = wristPose.position;
                // // Debug.Log("rigthHand pos: " + wristPosition);
                // xAxisPosition = wristPosition.x;
                // Debug.Log("x axis position: " + xAxisPosition);

                //RightIndex
                var indexTip = subsystem.rightHand.GetJoint(XRHandJointID.IndexTip);
                indexTip.TryGetPose(out var indexTipPose);
                Vector3 indexTipPosition = indexTipPose.position;

                //RightMiddle
                var middleTip = subsystem.rightHand.GetJoint(XRHandJointID.MiddleTip);
                middleTip.TryGetPose(out var middleTipPose);
                Vector3 middleTipPosition = middleTipPose.position;

                //RightRing
                var ringTip = subsystem.rightHand.GetJoint(XRHandJointID.RingTip);
                ringTip.TryGetPose(out var ringTipPose);
                Vector3 ringTipPosition = ringTipPose.position;

                //RightLittle
                var littleTip = subsystem.rightHand.GetJoint(XRHandJointID.LittleTip);
                littleTip.TryGetPose(out var littleTipPose);
                Vector3 littleTipPosition = littleTipPose.position;
                
                //RightThumb
                var ThumbTip = subsystem.rightHand.GetJoint(XRHandJointID.ThumbTip);
                ThumbTip.TryGetPose(out var ThumbTipPose);
                Vector3 ThumbTipPosition = ThumbTipPose.position;

                //LeftThumb
                var LThumbTip = subsystem.leftHand.GetJoint(XRHandJointID.ThumbTip);
                LThumbTip.TryGetPose(out var LThumbTipPose);
                Vector3 LThumbTipPosition = LThumbTipPose.position;
                
                //LeftThumb
                var LindexTip = subsystem.leftHand.GetJoint(XRHandJointID.IndexTip);
                LindexTip.TryGetPose(out var LIndexTipPose);
                Vector3 LIndexTipPosition = LIndexTipPose.position;

                 //LeftMiddle
                var LMiddleTip = subsystem.leftHand.GetJoint(XRHandJointID.MiddleTip);
                LMiddleTip.TryGetPose(out var LMiddleTipPose);
                Vector3 LMiddleTipPosition = LMiddleTipPose.position;
                
                //LeftRing
                var LRingTip = subsystem.leftHand.GetJoint(XRHandJointID.RingTip);
                LRingTip.TryGetPose(out var LRingTipPose);
                Vector3 LRingTipPosition = LRingTipPose.position;

                //LeftLittle
                var LLittleTip = subsystem.leftHand.GetJoint(XRHandJointID.LittleTip);
                LLittleTip.TryGetPose(out var LLittleTipPose);
                Vector3 LLittleTipPosition = LLittleTipPose.position;

                //Distance Variables
                float distance = Vector3.Distance(indexTipPosition, ThumbTipPosition);
                float distance2 = Vector3.Distance(middleTipPosition, ThumbTipPosition);
                float distance3 = Vector3.Distance(ringTipPosition, ThumbTipPosition);
                float distance4 = Vector3.Distance(littleTipPosition, ThumbTipPosition);
                float distance5 = Vector3.Distance(LIndexTipPosition, LThumbTipPosition);
                float distance6 = Vector3.Distance(LMiddleTipPosition, LThumbTipPosition);
                float distance7 = Vector3.Distance(LRingTipPosition, LThumbTipPosition);
                float distance8 = Vector3.Distance(LLittleTipPosition, LThumbTipPosition);
                
                //quick hack to set the distance out.
                xAxisPosition = distance;
                xAxisMiddlePosition = distance2;
                xAxisRingPosition = distance3;
                xAxisLittlePosition = distance4;
                xAxisLPosition = distance5;
                xAxisLMiddlePosition = distance6;
                xAxisLRingPosition = distance7;
                xAxisLLittlePosition = distance8;
                return;
            }
